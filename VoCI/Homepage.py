    ...
    if file_in is not None:
        if 'file_in' not in st.session_state or file_in != st.session_state['file_in']:
            ...
            try:
                ...
                try:
                    df_in = pd.read_csv(file_in, sep=None)
                    st.session_state['df_in'] = df_in
                except:
                     
                     st.error(f"Error: Insert an Excel or CSV file.")
...
def main():
    ...
    if "df_in" in st.session_state:
        ...
        if message_column:
            ...
            # Now it's safe to delete df_in from session_state if needed
            ...
            st.page_link('pages/1_Homepage_2.py', label='Add Data', icon='📥')
